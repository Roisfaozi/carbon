...
const isControlled =
  env.CONTROLLED_ENVIRONMENT === "true" || env.CONTROLLED_ENVIRONMENT === "1";

export const ERP_URL = isProd
  ? (env.ERP_URL ??
    (isControlled ? "https://itar.carbon.ms" : "https://app.carbon.ms"))
  : isPreview
    ? `https://${env.VERCEL_URL}`
    : (env.ERP_URL ?? "http://localhost:3000");

export const MES_URL = isProd
  ? (env.MES_URL ??
    (isControlled ? "https://mes.itar.carbon.ms" : "https://mes.carbon.ms"))
  : isPreview
    ? `https://${env.VERCEL_URL}`
    : (env.MES_URL ?? "http://localhost:3001");
